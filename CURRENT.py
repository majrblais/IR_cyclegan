...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i, img in enumerate(train_img.take(self.num_img)):
            ...
            prediction.save(
                "generated_decay_{i}_{epoch}.png".format(i=i, epoch=epoch + 1)
            )
        plt.show()
        plt.close()
...
lr=tf.keras.optimizers.schedules.ExponentialDecay(0.0001,decay_steps=1000,decay_rate=0.75,staircase=True)
...
# Callbacks
plotter = GANMonitor()
checkpoint_filepath = "./model_checkpoints/cyclegan_decay_checkpoints.{epoch:03d}"
model_checkpoint_callback = keras.callbacks.ModelCheckpoint(
    filepath=checkpoint_filepath
)
#lr=tf.keras.optimizers.schedule.ExponentialDecay(0.1,decay_steps=100,decay_rate=0.96,straicase=True)


# Here we will train the model for just one epoch as each epoch takes around
# 7 minutes on a single P100 backed machine.
#cycle_gan_model.load_weights('./model_checkpoints/cyclegan_checkpoints.015')
